        ...
        if([row.identifier isEqualToString:ORDER_VIEW_GIFTCODE]) {
            ...
            if(giftCodeCb.checkState == M13CheckboxStateChecked) {
                ...
                if(((NSArray *)[customerData objectForKey:@"list_code"]).count > 0){
                    ...
                    [existingCodePicker selectRow:0 inComponent:0 animated:NO];
                    existingCodeTextField.frame = CGRectMake(paddingX, cellY, viewWidth, 40);
                    cellY += 40 + paddingY;
                }
                SimiButton *applyGiftCodeButton = [[SimiButton alloc] initWithFrame:CGRectMake(paddingX + 10, cellY, viewWidth - 2*paddingX - 20, 40) title:@"Apply Gift Code" titleFont:[UIFont fontWithName:THEME_FONT_NAME_REGULAR size:FONT_SIZE_LARGE]];
            ...
            if(giftCardCreditCb.checkState == M13CheckboxStateChecked) {
                ...
                if(!giftCardCreditTextField) {
                    giftCardCreditTextField = [[SimiTextField alloc] initWithFrame:CGRectMake(paddingX, cellY, viewWidth, 40) placeHolder:@"" font:[UIFont fontWithName:THEME_FONT_NAME size:FONT_SIZE_LARGE] textColor:THEME_CONTENT_COLOR];
                ...
                SimiButton *applyGiftCardCreditButton = [[SimiButton alloc] initWithFrame:CGRectMake(paddingX + 10, cellY, viewWidth - 2*paddingX - 20, 40) title:@"Apply Credit" titleFont:[UIFont fontWithName:THEME_FONT_NAME_REGULAR size:FONT_SIZE_LARGE]];
...
- (void)applyGiftCode: (SimiButton *)button {
    ...
    if((![giftCodeTextField.text isEqualToString:@""] && giftCodeTextField != nil) || (![existingCodeTextField.text isEqualToString:@""] && existingCodeTextField != nil)) {
        ...
        if(![existingCodeTextField.text isEqualToString:@""] && existingCodeTextField != nil) {
            [params addEntriesFromDictionary:@{@"existed_giftcode":giftCodeSelected}];
            existingCodeTextField.text = @"";
        }
    }else {
        [orderVC showAlertWithTitle:@"" message:@"No gift card code selected"];
        return;
    }
...
- (void)applyGiftCardCredit: (SimiButton *)button {
    ...
    if(![giftCardCreditTextField.text isEqualToString:@""]){
        ...
        [orderVC showAlertWithTitle:@"" message:@"Please change amount"];
        return;
    }
...
- (void)didUseGiftCardCredit: (NSNotification *)noti {
    ...
    if(responder.status == SUCCESS) {
        ...
        [orderVC showAlertWithTitle:@"" message:responder.message completionHandler:^{
            [orderVC getOrderConfig];
        }];
    }
}
