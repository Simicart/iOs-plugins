//
//  SCCustomizeHomeViewController.m
//  SimiCartPluginFW
//
...
@interface SCCustomizeHomeViewController ()<MatrixBannerScrollViewDelegate>

@end

@implementation SCCustomizeHomeViewController
- (void)createCells{
    ...
    if (phoneBanners.count > 0) {
        ZaraHomeViewSection *bannerSection = [[ZaraHomeViewSection alloc] initWithIdentifier:ZARA_HOME_BANNER];
        [self.cells addObject:bannerSection];
        [bannerSection addRow:[[SimiRow alloc] initWithIdentifier:ZARA_HOME_BANNER height:SCALEVALUE(160)]];
    }
...
- (UITableViewCell *)createBannerCellForRow:(SimiRow *)bannerRow{
    ...
    if (cell == nil) {
        ...
        MatrixBannerScrollView* bannerScrollView = [[MatrixBannerScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, bannerRow.height)];
...
-(void) didTapInBanner:(SimiHomeBannerModel *) banner{
    ...
    if (banner.type == BannerCategoryInApp) {
        if (banner.categoryHasChildren) {
            [[SCAppController sharedInstance]openCategoryWithNavigationController:self.navigationController categoryId:banner.categoryId categoryName:banner.categoryName openCategoryFrom:CategoryOpenFromHomeScreen moreParams:nil];
        }else{
