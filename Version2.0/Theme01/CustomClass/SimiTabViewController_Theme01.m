//
//  SimiTabViewController_Theme01.m
//  Tan Hoang
//
...
#pragma mark - TabView
//@class TabView_Theme01;

@interface TabView_Theme01()
@property (nonatomic, getter = isSelected) BOOL selected;
@property (nonatomic) UIColor *indicatorColor;
@end

@implementation TabView_Theme01
...
#pragma mark - SimiTabViewController_Theme01
@interface SimiTabViewController_Theme01 () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
@implementation SimiTabViewController_Theme01
...
- (void)layoutSubviews {
    ...
    if([self tabFrame].size.height <= 0){
        ...
        self.tabsView.frame = [self tabFrame];
    }
    
    if([self contentFrame].size.height <= 0){
...
#pragma mark - Setters
- (void)setTabFrame:(CGRect)tabFrame
{
    _tabFrame = tabFrame;
}
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        if(i!= self.activeTabIndex){
            UIView *tabViewNotSelect = [self tabViewAtIndex:i];
            [tabViewNotSelect setBackgroundColor:[self tabsViewBackgroundColor]];
        }
    }
...
- (void)setActiveContentIndex:(NSUInteger)activeContentIndex {
    ...
    __weak SimiTabViewController_Theme01 *weakSelf = self;
...
- (NSNumber *)tabHeight {
    ...
    if (!_tabHeight) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabHeight withDefault:value];
...
- (NSNumber *)tabOffset {
    ...
    if (!_tabOffset) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabOffset withDefault:value];
...
- (NSNumber *)tabWidth {
    ...
    if (!_tabWidth) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabWidth withDefault:value];
...
- (NSNumber *)tabLocation {
    ...
    if (!_tabLocation) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabLocation withDefault:value];
...
- (NSNumber *)startFromSecondTab {
    ...
    if (!_startFromSecondTab) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionStartFromSecondTab withDefault:value];
...
- (NSNumber *)centerCurrentTab {
    ...
    if (!_centerCurrentTab) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionCenterCurrentTab withDefault:value];
...
- (NSNumber *)fixFormerTabsPositions {
    ...
    if (!_fixFormerTabsPositions) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionFixFormerTabsPositions withDefault:value];
...
- (NSNumber *)fixLatterTabsPositions {
    ...
    if (!_fixLatterTabsPositions) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:valueForOption:withDefault:)])
            value = [self.delegate simiTabView:self valueForOption:SimiTabViewOptionFixLatterTabsPositions withDefault:value];
...
- (UIColor *)indicatorColor {
    ...
    if (!_indicatorColor) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:colorForComponent:withDefault:)]) {
            color = [self.delegate simiTabView:self colorForComponent:SimiTabViewIndicator withDefault:color];
        }
...
- (UIColor *)tabsViewBackgroundColor {
    ...
    if (!_tabsViewBackgroundColor) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:colorForComponent:withDefault:)]) {
            color = [self.delegate simiTabView:self colorForComponent:SimiTabViewTabsView withDefault:color];
        }
...
- (UIColor *)contentViewBackgroundColor {
    ...
    if (!_contentViewBackgroundColor) {
        ...
        if ([self.delegate respondsToSelector:@selector(simiTabView:colorForComponent:withDefault:)]) {
            color = [self.delegate simiTabView:self colorForComponent:SimiTabViewContent withDefault:color];
        }
...
- (void)reloadData {
    ...
    // So that, SimiTabView will reflect the changes
    // Empty all options
    _tabHeight = nil;
    _tabOffset = nil;
    _tabWidth = nil;
...
- (void)selectTabAtIndex:(NSUInteger)index {
    ...
    if ([self.delegate respondsToSelector:@selector(simiTabView:didChangeTabToIndex:)]) {
        [self.delegate simiTabView:self didChangeTabToIndex:self.activeTabIndex];
    }
}
...
- (void)setNeedsReloadOptions {
    ...
    self.tabWidth = [NSNumber numberWithFloat:[self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabWidth withDefault:kTabWidth]];
    self.tabOffset = [NSNumber numberWithFloat:[self.delegate simiTabView:self valueForOption:SimiTabViewOptionTabOffset withDefault:kTabOffset]];
    self.centerCurrentTab = [NSNumber numberWithFloat:[self.delegate simiTabView:self valueForOption:SimiTabViewOptionCenterCurrentTab withDefault:kCenterCurrentTab]];
    self.fixFormerTabsPositions = [NSNumber numberWithFloat:[self.delegate simiTabView:self valueForOption:SimiTabViewOptionFixFormerTabsPositions withDefault:kFixFormerTabsPositions]];
    self.fixLatterTabsPositions = [NSNumber numberWithFloat:[self.delegate simiTabView:self valueForOption:SimiTabViewOptionFixLatterTabsPositions withDefault:kFixLatterTabsPositions]];
...
- (void)setNeedsReloadColors {
    ...
    // If it is, update it
    indicatorColor = [self.delegate simiTabView:self colorForComponent:SimiTabViewIndicator withDefault:kIndicatorColor];
    
    if (![self.indicatorColor isEqualToColor:indicatorColor]) {
        
        // We will iterate through all of the tabs to update its indicatorColor
        [self.tabs enumerateObjectsUsingBlock:^(TabView_Theme01 *tabView, NSUInteger index, BOOL *stop) {
    ...
    // If it is, update it
    tabsViewBackgroundColor = [self.delegate simiTabView:self colorForComponent:SimiTabViewTabsView withDefault:kTabsViewBackgroundColor];
    ...
    // Yeah update it, too
    contentViewBackgroundColor = [self.delegate simiTabView:self colorForComponent:SimiTabViewContent withDefault:kContentViewBackgroundColor];
...
- (UIColor *)colorForComponent:(SimiTabViewComponent)component {
    ...
    switch (component) {
        ...
        case SimiTabViewTabsView:
            return [self tabsViewBackgroundColor];
        case SimiTabViewContent:
...
- (void)defaultSetup {
    ...
    self.tabCount = [self.dataSource numberOfTabsForSimiTabView:self];
...
-(TabView_Theme01 *)roundCornersOnView:(TabView_Theme01 *)view onTopLeft:(BOOL)tl topRight:(BOOL)tr bottomLeft:(BOOL)bl bottomRight:(BOOL)br radius:(float)radius {
    ...
    if (tl || tr || bl || br) {
        ...
        TabView_Theme01 *roundedView = view;
...
- (TabView_Theme01 *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        UIView *tabViewContent = [self.dataSource simiTabView:self viewForTabAtIndex:index];
        ...
        TabView_Theme01 *tabView = [[TabView_Theme01 alloc] initWithFrame:CGRectMake(0.0, 0.0, [self.tabWidth floatValue], [self.tabHeight floatValue])];
        [tabView addSubview:tabViewContent];
        [tabView setClipsToBounds:YES];
        [tabView setIndicatorColor:[UIColor lightGrayColor]];
        if (index == 0) {
            CGRect frame = tabView.frame;
            frame.size.width = [self roundWith];
            frame.size.height = [self roundHeight];
            tabView = [self roundCornersOnView:tabView onTopLeft:YES topRight:NO bottomLeft:YES bottomRight:NO radius:frame.size.width];
            
            UIImage *roundLeft = [UIImage imageNamed:[self roundLeftImageName]];
            UIImageView *roundLeftImage = [[UIImageView alloc]init];
            roundLeftImage.image = roundLeft;
            roundLeftImage.frame = frame;
            [tabView addSubview:roundLeftImage];
        }else if(index == self.tabCount - 1){
            CGRect frame = tabView.frame;
            frame.size.width = [self roundWith];
            frame.size.height = [self roundHeight];
            frame.origin.x += tabView.frame.size.width - frame.size.width;
            tabView = [self roundCornersOnView:tabView onTopLeft:NO topRight:YES bottomLeft:NO bottomRight:YES radius:frame.size.width];
            
            UIImage *roundRight = [UIImage imageNamed:[self roundRightImageName]];
            UIImageView *roundRightImage = [[UIImageView alloc]init];
            roundRightImage.image = roundRight;
            roundRightImage.frame = frame;
            [tabView addSubview:roundRightImage];
        }else{
            [tabView.layer setBorderColor:[UIColor lightGrayColor].CGColor];
            [tabView.layer setBorderWidth:0.1f];
        }
...
- (UIViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    if ([[self.contents objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        if ([self.dataSource respondsToSelector:@selector(simiTabView:contentViewControllerForTabAtIndex:)]) {
            viewController = [self.dataSource simiTabView:self contentViewControllerForTabAtIndex:index];
        } else if ([self.dataSource respondsToSelector:@selector(simiTabView:contentViewForTabAtIndex:)]) {
            
            UIView *view = [self.dataSource simiTabView:self contentViewForTabAtIndex:index];
