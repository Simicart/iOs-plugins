...
@interface ZThemeOrderViewController ()

@end

@implementation ZThemeOrderViewController
...
{
    self.title = SCLocalizedString(@"Review Order");
...
- (void)viewWillAppearBefore:(BOOL)animated
{
    ...
    self.isNeedReloadCart = YES;
    if (!self.isSavingShippingMethod) {
        [self getOrderConfig];
    }
    ...
    if(self.paymentCollection.count > 0){
        ...
        for(int i=0; i<self.paymentCollection.count; i++){
            if (i == self.selectedPayment){
                ...
                CGSize neededSize = [content sizeWithFont:[UIFont fontWithName:ZTHEME_FONT_NAME_REGULAR size:15] constrainedToSize:maxSize lineBreakMode:NSLineBreakByTruncatingTail];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([simiSection.identifier isEqualToString:ORDER_ADDRESS_SECTION]) {
        if(simiRow.identifier == ORDER_VIEW_BILLING_ADDRESS){
            ...
            if (self.billingAddress != nil) {
                ...
                userLabel.font = [UIFont fontWithName:ZTHEME_FONT_NAME_LIGHT size:16.0];
                ...
                addressLabel.font = [UIFont fontWithName:ZTHEME_FONT_NAME_LIGHT size:16.0];
    ...
    }else if ([simiSection.identifier isEqualToString:ORDER_PAYMENT_SECTION])
    {
        if(simiRow.identifier == ORDER_VIEW_PAYMENT_METHOD){
            ...
            UILabel *lblContentPayment = [[UILabel alloc]initWithFrame:CGRectMake(40, 40, 250, 40)];
            ...
            paymentTitleLabel.font = [UIFont fontWithName:ZTHEME_FONT_NAME_LIGHT size:16.0];
            paymentTitleLabel.textColor = [UIColor blackColor];
            [paymentTitleLabel removeFromSuperview];
            //  Liam Update RTL
            if ([[SimiGlobalVar sharedInstance]isReverseLanguage]) {
        ...
        if(simiRow.identifier == ORDER_VIEW_COUPONCODE){
            ...
            if (cell == nil) {
                ...
                if (self.textFieldCouponCode == nil) {
                    ...
                    [self.textFieldCouponCode setFont:[UIFont fontWithName:ZTHEME_FONT_NAME_REGULAR size:16]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(simiRow.identifier == ORDER_VIEW_BILLING_ADDRESS){
        ...
        if ([[payment valueForKey:@"show_type"] integerValue] == PaymentShowTypeCreditCard) {
            NSArray *creditCardTypes = [[self.paymentCollection objectAtIndex:indexPath.row] valueForKey:@"cc_types"];
            if (creditCardTypes != nil) {
                ...
                nextController.delegate = self;
                if (_creditCard != nil) {
                    nextController.creditCard = _creditCard;
                }
                nextController.creditCardList = [payment valueForKey:@"cc_types"];
                nextController.isUseCVV = [[payment valueForKey:@"useccv"] boolValue];
                if([[[self.paymentCollection objectAtIndex:self.selectedPayment] valueForKey:@"payment_method"] isEqualToString:@"AUTHORIZENET"])
                {
                    if (self.creditCardAuthorize != nil) {
                        nextController.defaultCard = [[NSDictionary alloc]initWithObjectsAndKeys:
                                                      [self.creditCardAuthorize valueForKey:@"card_type"],@"card_type",
                                                      [self.creditCardAuthorize valueForKey:@"card_number"],@"card_number",
                                                      [self.creditCardAuthorize valueForKey:@"expired_month"], @"expired_month",
                                                      [self.creditCardAuthorize valueForKey:@"expired_year"], @"expired_year",
                                                      [self.creditCardAuthorize valueForKey:@"cc_id"] , @"cc_id", nil];
                    }
                }else if ([[[self.paymentCollection objectAtIndex:self.selectedPayment] valueForKey:@"payment_method"] isEqualToString:@"CCSAVE"])
                {
                    if (self.creditCardSaved != nil) {
                        nextController.defaultCard = [[NSDictionary alloc]initWithObjectsAndKeys:
                                                      [self.creditCardSaved valueForKey:@"card_type"],@"card_type",
                                                      [self.creditCardSaved valueForKey:@"card_number"],@"card_number",
                                                      [self.creditCardSaved valueForKey:@"expired_month"], @"expired_month",
                                                      [self.creditCardSaved valueForKey:@"expired_year"], @"expired_year",
                                                      nil];
                    }
                }
                */
...
- (void)placeOrder{
    if (self.selectedPayment >= 0) {
        ...
        /*
        if (self.creditCardAuthorize != nil && [[[self.paymentCollection objectAtIndex:self.selectedPayment] valueForKey:@"payment_method"] isEqualToString:@"AUTHORIZENET"]) {
            for (NSString *key in [self.creditCardAuthorize allKeys]) {
                [params setValue:[self.creditCardAuthorize valueForKey:key] forKey:key];
            }
        }
        if (self.creditCardPaypalDirect != nil && [[[self.paymentCollection objectAtIndex:self.selectedPayment] valueForKey:@"payment_method"] isEqualToString:@"PAYPAL_DIRECT"]) {
            for (NSString *key in [self.creditCardPaypalDirect allKeys]) {
                [params setValue:[self.creditCardPaypalDirect valueForKey:key] forKey:key];
            }
        }
        if (self.creditCardSaved != nil && [[[self.paymentCollection objectAtIndex:self.selectedPayment] valueForKey:@"payment_method"] isEqualToString:@"CCSAVE"]) {
            for (NSString *key in [self.creditCardSaved allKeys]) {
                [params setValue:[self.creditCardSaved valueForKey:key] forKey:key];
            }
        }
        */
        //  End Update Credit Card
