...
{
    /*
    SCStoreListTableViewCell *cell = (SCStoreListTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"SCStoreListTableViewCell"];
    if (cell==nil) {
        NSArray *arrayNib = [[NSBundle mainBundle] loadNibNamed:@"SCStoreListTableViewCell" owner:self options:nil];
        cell = [arrayNib objectAtIndex:0];
        
    }
    cell.delegate = self;
    cell.storeLocatorModel = [sLModelCollection objectAtIndex:indexPath.row];
    [cell initStoreListCell];
     */
    ...
    switch (self.listViewOption) {
        case ListViewOptionNoneSearch:
            ...
            if (isFirstRun || offset != [sLModelCollection count]) {
                isFirstRun = NO;
                offset = [sLModelCollection count];
                [sLModelCollection getStoreListWithLatitude:[NSString stringWithFormat:@"%f",currentLatitube] longitude:[NSString stringWithFormat:@"%f",currentLongitube] offset:[NSString stringWithFormat:@"%d",(int)offset] limit:@"5"];
