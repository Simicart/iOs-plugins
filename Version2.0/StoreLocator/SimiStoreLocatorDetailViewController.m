...
- (void)viewDidLoadBefore
{
    ...
    float widthContent = CGRectGetWidth(scrView.frame);
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        widthContent = 680;
    }
    [scrView setBackgroundColor:[UIColor whiteColor]];
    [self.view addSubview:scrView];
#pragma mark Store name
    heightScrViewContentSize = 5;

    float sizeIcon = 20; // sice icon
    float sizeStoreImage = 100; //  size map
    float space = 5; // Khoang cach line cac label
    float edgeSpace = 10; // Khoang cach hai mep view
    ...
    [lblStoreName setFont:[UIFont fontWithName:[NSString stringWithFormat:@"%@-%@",THEME_FONT_NAME,@"Bold"] size:THEME_FONT_SIZE + 4]];
...
#pragma mark Phone
    if (!([[sLModel valueForKey:@"phone"] isEqualToString:@""] || [sLModel valueForKey:@"phone"] == nil || [[sLModel valueForKey:@"phone"] isKindOfClass:[NSNull class]])) {
        ...
        [lblStorePhone setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
...
#pragma mark Email
    if (!([[sLModel valueForKey:@"email"] isEqualToString:@""] || [sLModel valueForKey:@"email"] == nil || [[sLModel valueForKey:@"email"] isKindOfClass:[NSNull class]])) {
        ...
        [lblStoreEmail setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
...
#pragma mark Web Site
    if (!([[sLModel valueForKey:@"link"] isEqualToString:@""]|| [sLModel valueForKey:@"link"] == nil || [[sLModel valueForKey:@"link"] isKindOfClass:[NSNull class]]))
    {
        ...
        [lblStoreWebSite setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
...
#pragma mark Information
    if (!([[sLModel valueForKey:@"description"] isEqualToString:@""]||[sLModel valueForKey:@"description"] == nil || [[sLModel valueForKey:@"description"] isKindOfClass:[NSNull class]])) {
        ...
        [lblInfomationContents setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
        ...
        if (lblInfomationContents.expectedHeight > (heightLabel*2)) {
            ...
            [btnShowMoreLess.titleLabel setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
            isShowMore = YES;
...
#pragma mark Open Hour
    ...
    float labelTitleX = sizeIcon + 10;
    float widthTitle = 100;
    float labelValueX = labelTitleX + widthTitle +10;
    float widthValue = 150;
    float heightOpenHourView = 5;
    ...
    [lblOpenHours setFont:[UIFont fontWithName:[NSString stringWithFormat:@"%@-%@",THEME_FONT_NAME,@"Bold"] size:THEME_FONT_SIZE + 3]];
    ...
    for (UIView *view in viewOpenHours.subviews) {
        if ([view isKindOfClass:[UILabel class]]) {
            ...
            if (![(NSString*)label.simiObjectIdentifier isEqualToString:openHours] ) {
                [label setFont:[UIFont fontWithName:THEME_FONT_NAME size:THEME_FONT_SIZE - 3]];
...
#pragma mark Special Day
    ...
    if (arraySpecialDays.count > 0) {
        ...
        [lblSpecialsDay setFont:[UIFont fontWithName:[NSString stringWithFormat:@"%@-%@",THEME_FONT_NAME,@"Bold"] size:THEME_FONT_SIZE + 3]];
...
#pragma mark Holidays
    ...
    if (arrayHolidays.count > 0) {
        ...
        [lblHolidays setFont:[UIFont fontWithName:[NSString stringWithFormat:@"%@-%@",THEME_FONT_NAME,@"Bold"] size:THEME_FONT_SIZE + 3]];
