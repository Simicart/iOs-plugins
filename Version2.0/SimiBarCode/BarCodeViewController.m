...
#define ImageCanvasFrame UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone? CGRectMake(0, 150, 320, 270):CGRectMake(337, 200, 350, 350)
#define ButtonFlashFrame  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone?CGRectMake(60, 90, 80, 30):CGRectMake(0, 0, 0, 0)
#define ButtonPhotoFrame  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone?CGRectMake(190, 90, 80, 30):CGRectMake(880, 90, 80, 30)
#define ImageBackgroundHeader UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone? CGRectMake(0, 0, 320, 150):CGRectMake(0, 60, 1024, 140)
#define ImageBackgroundFooter UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone? CGRectMake(0, 420, 320, 200):CGRectMake(0, 550, 1024, 218)
...
{
    CGRect frame = _leftMenuView.frame;
    frame.origin.x -= 70;
    [_leftMenuView setFrame:frame];
}
        ...
        if ([[responder.status uppercaseString] isEqualToString:@"SUCCESS"]) {
            if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
                switch ([SimiGlobalVar sharedInstance].themeUsing) {
                    case ThemeShowDefault:
                    {
                        ...
                        productViewController.productId = [_barCodeModel valueForKey:@"product_id"];
                        [self.navigationController pushViewController:productViewController animated:YES];
                    }
                        break;
                    case ThemeShowMatrixTheme:
                    {
                        id themeViewController = [NSClassFromString(@"SCProductViewController_Theme01") new];
                        SCProductViewController *productViewController = (SCProductViewController*)themeViewController;
                        productViewController.productId = [_barCodeModel valueForKey:@"product_id"];
                        [self.navigationController pushViewController:productViewController animated:YES];
                    }
                    default:
            ...
            }else
            {
                switch ([SimiGlobalVar sharedInstance].themeUsing) {
                    case ThemeShowDefault:
                    {
                        SCProductViewController_Pad *productViewController = [SCProductViewController_Pad new];
                        productViewController.productId = [_barCodeModel valueForKey:@"product_id"];
                        productViewController.isInPopover = YES;
                        [productViewController getProduct];
                        self.popoverShowProduct = [[UIPopoverController alloc] initWithContentViewController:[[UINavigationController alloc]initWithRootViewController:productViewController]];
                        productViewController.navigationController.navigationBar.hidden = YES;
                        [self.popoverShowProduct setDelegate:self];
                        CGRect rect = CGRectMake(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 1, 1);
                        if (SIMI_SYSTEM_IOS > 7.0){
                            [self.popoverShowProduct setBackgroundColor:[UIColor clearColor]];
                        }
                        [self.popoverShowProduct presentPopoverFromRect:rect inView:self.view permittedArrowDirections:0 animated:YES];
                        [productViewController setPopOver:_popoverShowProduct];
                        if (SYSTEM_VERSION_LESS_THAN(@"7.0")) {
                            productViewController.contentSizeForViewInPopover = CGSizeMake(768, 768);
                        } else{
                            productViewController.preferredContentSize = CGSizeMake(768, 768);
                        }
                    }
                        break;
                    case ThemeShowMatrixTheme:
                    {
                        id themeViewController = [NSClassFromString(@"SCProductViewControllerPad_Theme01") new];
                        SCProductViewController *productViewController = (SCProductViewController*)themeViewController;
                        productViewController.productId = [_barCodeModel valueForKey:@"product_id"];
                        [self.navigationController pushViewController:productViewController animated:YES];
                    }
